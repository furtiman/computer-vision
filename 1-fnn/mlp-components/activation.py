...
def relu(prime: bool, in_ft: np.array) -> np.array:
    ''' ReLU: f(x) = max(0, x) '''
    if not prime:
...
def softplus(prime: bool, in_ft: np.array) -> np.array:
...
def sigmoid(prime: bool, in_ft: np.array) -> np.array:
    ''' Sigmoid: f(x) = 1 / 1 + e^(-x) '''
    if not prime:
...
def softmax(prime: bool, in_ft: np.array) -> np.array:
    '''
